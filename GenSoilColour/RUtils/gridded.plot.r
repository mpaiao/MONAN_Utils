...
gridded.plot <<- function( x                = seq(from=0,to=1,len=nrow(z))
   ...
   if (missing(z)) {
      ...
      if (! missing(x)) {
         if (is.list(x)) {
            #--- Variable 'x' is a list, copy the elements to variables.
            z = x$z
            y = x$y
            x = x$x
            #---~---
         }else{
            #--- Variable 'x' is an array, make up some x axis.
            z = x
         ...
         #--- Variable 'z' was not given, abort run.
         stop("Variable \"z\" is missing and \"x\" does not have an element named \"z\".")
         #---~---
      }#end if (! missing(x))
      #---~---
   }else if (is.list(x)) {
      #--- Variable 'z' provided, check whether x and y were given as a list.
      y = x$y
      x = x$x
      #---~---
   }#end if (missing(z))
   ...
   if (! (is.matrix(z) || is.data.frame(z) || is.data.table(z))){
      stop("Variable/element \"z\" must be a matrix or a data.frame/data.table.")
   }#end if (! (is.matrix(z) || is.data.frame(z) || is.data.table(z)))
   #---~---


   #--- Axes 'x' and 'y' must continuously increase.
   if (any(diff(x) %le% 0) || any(diff(y) %le% 0)){
       stop(" Values in axes \"x\" and \"y\" must always increase.")
   }#end if (any(diff(x) %le% 0) || any(diff(y) %le% 0))
   #---~---
   ...
   if ( plot.legend && (! key.vertical)){
      warning(" key.vertical=FALSE ignored because the plot requires a legend too.")
      key.vertical = TRUE
   }#end if ( plot.legend && (! key.vertical))
   #---~---
   ...
   if ( (! is.null(oma)) && (! is.null(omd))){
      ...
      par(omd=omd)
   }#end if ( (! is.null(oma)) && (! is.null(omd)))
   #---~---
   ...
   for (a in sequence(n.after)){
      ...
      this.after    = plot.after[[a]]
      this.function = names(plot.after)[[a]]
      #---~---
      
      if ("use.package" %in% names(this.after)){
         this.package           = this.after$use.package
         this.eptfun            = paste0(this.package,"::",this.function)
         this.eptfun            = eval( parse( text = this.eptfun ) )
         this.function          = "this.eptfun"
         this.after$use.package = NULL
      }#end if (use.package %in% names(this.after))

      #--- Call function.
      do.call(what=this.function,args=this.after)
      #---~---
